...
class WebappToken(_Base):
    ...
    def verify(token):
        ...
        try:
            entry = session.query(WebappToken).filter(WebappToken.token == token).one()
    ...
    def _invalidate_tokens(session, login):
        try:
            entries = session.query(WebappToken).\
                filter(WebappToken.login == login).\
                filter(WebappToken.revoked.is_(False)).\
                all()
        except NoResultFound:
            return
        except SQLAlchemyError:
